 ...
 */
Vkcmdpool::Vkcmdpool(
    const Vkdev *device) : m_device(device->get())
{
...
Vkbuf::Vkbuf(
    ...
    VkBufferUsageFlags bufferUsage, bool exportCapable) : m_device(device->get())
{
    VkResult result = VK_SUCCESS;
...
Vkimg2d::Vkimg2d(
    ...
    bool exportCapable) : m_device(device->get()), m_extent(extent)
{
    VkResult result = VK_SUCCESS;
...
Vkdevicemem::Vkdevicemem(
    ...
    VkMemoryPropertyFlags memoryProperties, bool exportCapable) : m_device(device->get()), m_size(size)
{
    VkResult result = VK_SUCCESS;
...
Cudactx::Cudactx(const Vkdev *device)
{
    ...
    result = cuCtxCreate(&m_context, &ctxParams, 0, dev);

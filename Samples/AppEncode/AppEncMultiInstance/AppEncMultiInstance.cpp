	...
	while (encoderWorking)
	{
		if (encodeQueue.size())
		{
			...
			for (uint32_t i = 0; i < enc.numFrames; i++)
			{
				...
				for (uint32_t j = 0; j < encOutBuf.size(); ++j)
				{ // gather encoded data in output buffer to write to file later
...
void asyncFwrite(ConcurrentQueue<fileWriteData> &fwriteQueue, std::atomic<bool> &fwriteWorking)
{
	...
	while (fwriteWorking)
	{
		if (fwriteQueue.size())
		{
			...
			{
				if (output.isAV1 && output.vidThreadIdx > 0 && output.isFirst && firstPackage)
				{																								 // for AV1 we only need an IVF header on the first video portion
...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		if (numFramesPerVideoPortion == 0)
		{ // number of video frames per video portion cannot be 0
		...
		CUcontext cuContext;
		ck(cuCtxCreate(&cuContext, /*ctxCreateParams*/ nullptr,
					   CU_CTX_SCHED_BLOCKING_SYNC, cuDevice)); // Create single CUDA context
		...
		if (numFramesPerVideoPortion > numFramesTotal)
		{ // the number of frames per video portion should not be larger than the total number of frames

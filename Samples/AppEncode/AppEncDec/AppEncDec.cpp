...
void ShowHelpAndExit(const char *szBadOption = NULL)
{
    ...
    oss << "Options:" << std::endl
        ...
        << "-if          Input format: iyuv nv12 nv16 p010 p210 bgra bgra64" << std::endl
        << "-of          Output format: native(nv12/p010) bgra bgra64" << std::endl
        << "-gpu         Ordinal of GPU to use" << std::endl;
    oss << NvEncoderInitParam().GetHelpMessage(false, false, true, false, false, true);
    if (bThrowError)
    {
                ...
                bool bBgra64, const char *szInFilePath, const char *szMediaPath, std::exception_ptr &encExceptionPtr)
{
    ...
    try
    {
        ck(cuCtxCreate(&cuContext, /*ctxCreateParams*/ nullptr, 0, cuDevice));
        ...
        FFmpegStreamer streamer(pEncodeCLIOptions->IsCodecH264() ? AV_CODEC_ID_H264 : pEncodeCLIOptions->IsCodecHEVC() ? AV_CODEC_ID_HEVC
                                                                                                                       : AV_CODEC_ID_AV1,
                                nWidth, nHeight, 25, szMediaPath);
        do
        {
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (!*szOutFilePath)
        {
            sprintf(szOutFilePath, "out.%s", eOutputFormat != native ? vstrOutputFormatName[eOutputFormat].c_str() : (eInputFormat != NV_ENC_BUFFER_FORMAT_YUV420_10BIT ? "nv12" : "p010"));
        }

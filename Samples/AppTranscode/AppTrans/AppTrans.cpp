...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (!*szOutFilePath)
        {
            sprintf(szOutFilePath, encodeCLIOptions.IsCodecH264() ? "out.h264" : encodeCLIOptions.IsCodecHEVC() ? "out.hevc"
                                                                                                                : "out.av1");
        }
        ...
        else
        {
            muxer = std::unique_ptr<FFmpegMuxer>(new FFmpegMuxer(szOutFilePath, mediaFormat, demuxer.GetAVFormatContext(),
                                                                 encodeCLIOptions.IsCodecH264() ? AV_CODEC_ID_H264 : encodeCLIOptions.IsCodecHEVC() ? AV_CODEC_ID_HEVC
                                                                                                                                                    : AV_CODEC_ID_AV1,
        ...
        std::cout << "Total frame transcoded: " << nFrame << std::endl

...
static void createCudaContext(CUcontext *cuContext, int iGpu, unsigned int flags)
{
    ...
    ck(cuCtxCreate(cuContext, nullptr, flags, cuDevice));
}
...
static void ShowDecoderCapability()
{
    ...
    for (int iGpu = 0; iGpu < nGpu; iGpu++)
    {
        ...
        for (int i = 0; i < sizeof(queryList) / sizeof(queryList[0]); i++)
        {
            ...
            std::cout << "Codec  " << std::left << std::setw(7) << queryList[i].aszCodecName << "BitDepth  " << std::setw(4) << decodeCaps.nBitDepthMinus8 + 8 << "ChromaFormat  " << std::setw(7) << aszChromaFormat[decodeCaps.eChromaFormat] << "Supported  " << std::setw(3) << (int)decodeCaps.bIsSupported << "MaxWidth  " << std::setw(7) << decodeCaps.nMaxWidth << "MaxHeight  " << std::setw(7) << decodeCaps.nMaxHeight << "MaxMBCount  " << std::setw(10) << decodeCaps.nMaxMBCount << "MinWidth  " << std::setw(5) << decodeCaps.nMinWidth << "MinHeight  " << std::setw(5) << decodeCaps.nMinHeight << "SurfaceFormat  " << std::setw(11) << strOutputFormats << std::endl;
        }
